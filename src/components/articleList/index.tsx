import React, { Fragment, useCallback, useMemo, useState, useEffect, useRef } from 'react';
import { Link } from 'react-router-dom';
import { Image, message } from 'antd';
import {
  PushpinTwoTone,
  CalendarOutlined,
  ContainerOutlined,
  TagOutlined,
  LoadingOutlined,
} from '@ant-design/icons';
import { useAppSelector, useAppDispatch } from '@/hooks/redux';
import { updateArticleList, updateCurrent } from '@/store/actions/home';
import { ArticleListType } from '@/store/actions/home';
import { articleListRequest, ArticleListParams } from '@/apis/article';
import { RequestConfig } from '@/utils/commonTypes';
import { RequestError } from '@/utils/request';
import * as MarkdownIt from 'markdown-it';
import dayjs from 'dayjs';
import { markdownFilter, checkVisible } from './util';
import { throttle } from 'lodash';
import './index.scss';

const ArticleList: React.FC = () => {
  const articleList: ArticleListType = useAppSelector((state) => state.articleList);
  const homeCurrent: number = useAppSelector((state) => state.homeCurrent);
  const [isLoading, setLoading] = useState<boolean>(true);
  const md = useMemo(() => new MarkdownIt(), [MarkdownIt]);
  const lastArticleRef = useRef<HTMLAnchorElement>(null);
  const onScroll = useRef(
    throttle(() => {
      const node = lastArticleRef.current;
      if (checkVisible(node)) {
        dispatch(updateCurrent());
      }
    }, 200)
  );

  const dispatch = useAppDispatch();
  const getArticleList = useCallback(async () => {
    const config: RequestConfig<ArticleListParams> = {
      api: '/api/article/listArticlePreviewPage',
      method: 'get',
      params: {
        current: homeCurrent,
        size: 10,
      },
    };
    try {
      setLoading(true);
      const response = await articleListRequest(config);
      const { articleList } = response;
      dispatch(updateArticleList(articleList));
      if (articleList.length < 1) {
        cancelScroll();
      }
    } catch (e) {
      const err: RequestError = e as RequestError;
      message.error(err.message);
    }
    setLoading(false);
  }, [homeCurrent]);

  useEffect(() => {
    getArticleList();
  }, [homeCurrent]);

  const cancelScroll = () => {
    window.removeEventListener('scroll', onScroll.current);
  };

  const addScroll = () => {
    window.addEventListener('scroll', onScroll.current);
  };

  useEffect(() => {
    addScroll();
    return () => {
      cancelScroll();
    };
  }, []);

  return (
    <Fragment>
      {articleList.map((articleItem, articleIndex) => (
        <div className='article-item-container' key={articleItem.id}>
          <Link className='image-container' to={`/article/${articleItem.id}`}>
            <Image
              className='article-cover'
              preview={false}
              width={'100%'}
              height={'100%'}
              src={`${articleItem.articleCover}?imageMogr2/rquality/20`}
              fallback='data:image/png;base64,'
            />
          </Link>
          <div className='article-content'>
            {/* 绑定最后一个element添加监听事件 */}
            {articleIndex === articleList.length - 1 ? (
              <Link
                to={`/article/${articleItem.id}`}
                className='article-title'
                ref={lastArticleRef}>
                {articleItem.articleTitle}
              </Link>
            ) : (
              <Link className='article-title' to={`/article/${articleItem.id}`}>
                {articleItem.articleTitle}
              </Link>
            )}
            <div className='mark-box'>
              {articleItem.isTop ? (
                <span className='mark-item sticky-mark'>
                  <PushpinTwoTone twoToneColor='#ff7242' />
                  置顶
                </span>
              ) : null}
              <span className='mark-item date-mark'>
                <CalendarOutlined />
                {dayjs(articleItem.createTime).format('YYYY-MM-DD')}
              </span>
              <span className='mark-item category-mark'>
                <ContainerOutlined />
                {articleItem.categoryName}
              </span>
              <span className='mark-item tag-list'>
                {articleItem.tagList.map((tagItem) => (
                  <span className='tag-item' key={tagItem.tagId}>
                    <TagOutlined />
                    {tagItem.tagName}
                  </span>
                ))}
              </span>
            </div>
            <Link to={`/article/${articleItem.id}`} className='article-brief'>
              {markdownFilter(md.render(articleItem.articlePreviewContent))}
            </Link>
          </div>
        </div>
      ))}
      {isLoading ? (
        <div className='article-list-loading'>
          <LoadingOutlined />
        </div>
      ) : null}
    </Fragment>
  );
};

export default ArticleList;
